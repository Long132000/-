...
class AuthWindow(tk.Tk):
    ...
    def login(self):
        ...
        cursor.execute('''
            SELECT * FROM users 
            WHERE email=? AND role=? AND password_hash=? AND status='active'
        ...
        if user:
            ...
            messagebox.showerror("Ошибка", "Неверные данные или аккаунт заблокирован")
...
class MainWindow(tk.Tk):
    ...
    def build_users_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="Обновить", command=self.load_users_data).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Заблокировать", command=self.block_user).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Разблокировать", command=self.unblock_user).pack(side="left", padx=5)
    ...
    def load_users_data(self):
        ...
        conn = sqlite3.connect('shop.db')
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, email, role, status FROM users")

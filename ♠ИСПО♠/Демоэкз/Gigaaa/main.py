...
MAX_IMAGE_SIZE = (80, 80)

# ------------------- ФУНКЦИИ ВАЛИДАЦИИ -------------------
def validate_date(date_str):
    """Проверяет, что дата существует (ГГГГ-ММ-ДД)."""
    if not date_str:
        return False
...
def validate_price(price):
    """Цена: не отрицательная, не более 1 000 000."""
    try:
        p = float(price)
        return 0 <= p <= 1000000
    except:
...
def validate_quantity(qty):
    """Количество: не отрицательное, не более 100 000."""
    try:
        q = int(qty)
        return 0 <= q <= 100000
    except:
...
def validate_name(name):
    """Имя/название: не пустое, минимум 2 символа."""
    return name and len(name.strip()) >= 2

def validate_address(address):
    """Адрес: не пустой, минимум 5 символов."""
    return address and len(address.strip()) >= 5

def validate_order_number(order_num):
    """Артикул заказа: не пустой, формат ORD-XXX или любой."""
    return order_num and len(order_num.strip()) >= 3

def validate_phone(phone):
    """Телефон: формат +7XXXXXXXXXX или 8XXXXXXXXXX (опционально)."""
    if not phone:
...
def validate_email(email):
    """Email: простой формат (опционально)."""
    if not email:
        return True  # необязательное поле
...
class App:
    ...
    def create_product_card(self, row):
        ...
        card = tk.Frame(self.scrollable_frame, relief=tk.RAISED, borderwidth=1, bg=bg_color)
        
        photo_frame = tk.Frame(card, bg=bg_color)
        ...
        photo_label = tk.Label(photo_frame, image=photo_img, bg=bg_color)
        photo_label.pack()
        
        info_frame = tk.Frame(card, bg=bg_color)
        info_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        name_label = tk.Label(info_frame, text=row['name'], font=('Arial', 12, 'bold'), bg=bg_color, anchor='w')
        name_label.pack(anchor='w')
        
        cat_label = tk.Label(info_frame, text=f"🏷️ {row['category_name']} | {row['supplier_name']}", bg=bg_color, anchor='w')
        ...
        if discount > 0:
            ...
            old_price = tk.Label(price_frame, text=f"{price:.2f} ₽", foreground='red', font=('Arial', 10, 'overstrike'), bg=bg_color)
            old_price.pack(side=tk.LEFT)
            new_price = tk.Label(price_frame, text=f" {final_price:.2f} ₽", foreground='black', font=('Arial', 11, 'bold'), bg=bg_color)
            new_price.pack(side=tk.LEFT)
            discount_label = tk.Label(info_frame, text=f"Скидка: {discount:.0f}%", foreground='green', bg=bg_color)
            discount_label.pack(anchor='w')
        else:
            price_label = tk.Label(info_frame, text=f"💰 {price:.2f} ₽", font=('Arial', 11), bg=bg_color)
            price_label.pack(anchor='w')
        
        qty_label = tk.Label(info_frame, text=f"📦 В наличии: {row['quantity']} {row['unit'] or 'шт'}", bg=bg_color)
        qty_label.pack(anchor='w')
        
        if row['description']:
            desc_label = tk.Label(info_frame, text=f"📝 {row['description'][:100]}...", wraplength=300, bg=bg_color, justify=tk.LEFT)
        ...
        if self.current_user and self.current_user['role'] == 'admin':
            ...
            del_btn = tk.Button(btn_frame, text="🗑️", width=3,
    ...
    def save_product(self):
        ...
        try:
            ...
            if not validate_quantity(quantity):
                raise ValueError("Количество должно быть от 0 до 100 000")
    ...
    def save_order(self):
        ...
        try:
            ...
            if not validate_order_number(order_number):
                raise ValueError("Артикул заказа должен содержать минимум 3 символа")
            ...
            if not validate_date(order_date):
                raise ValueError("Дата заказа должна быть в формате ГГГГ-ММ-ДД и существовать (например, 2025-12-31)")
            
            delivery_date = self.delivery_var.get().strip()
            if delivery_date:
                if not validate_date(delivery_date):
                    raise ValueError("Дата доставки должна быть в формате ГГГГ-ММ-ДД и существовать")
            else:
                delivery_date = None
...
def export_schema_to_sql():
    """Экспортирует схему БД в SQL файл."""
    try:
        conn = get_db_connection()
        with open('database_schema.sql', 'w', encoding='utf-8') as f:
            f.write("-- ============================================\n")
            f.write("-- СКРИПТ БАЗЫ ДАННЫХ\n")
            f.write("-- Создан: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n")
            f.write("-- ============================================\n\n")
            
            # Получаем все CREATE TABLE запросы
            tables = conn.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").fetchall()
            for table in tables:
                if table['sql']:
                    f.write(table['sql'] + ";\n\n")
            
            # Индексы
            indexes = conn.execute("SELECT sql FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'").fetchall()
            for idx in indexes:
                if idx['sql']:
                    f.write(idx['sql'] + ";\n\n")
            
            # Триггеры (если есть)
            triggers = conn.execute("SELECT sql FROM sqlite_master WHERE type='trigger'").fetchall()
            for tr in triggers:
                if tr['sql']:
                    f.write(tr['sql'] + ";\n\n")
        
        conn.close()
        print("✅ Скрипт БД сохранён в database_schema.sql")
        print(f"📁 Файл: {os.path.abspath('database_schema.sql')}")
    except Exception as e:
        print(f"❌ Ошибка: {e}")

# Запустите эту функцию один раз:
export_schema_to_sql()

...
namespace TextFileLibrary
{
    ...
    public class TextFile : IComparable<TextFile>
    {
        ...
        /// <summary>
        /// Поиск слов, содержащих заданное сочетание букв в начале или конце
        /// </summary>
        /// <param name="substring">Искомая подстрока</param>
        /// <returns>Список найденных слов</returns>
        public List<string> FindWordsWithSubstring(string substring)
        {
            ...
            {
                if (word.StartsWith(substring, StringComparison.OrdinalIgnoreCase) ||
                    word.EndsWith(substring, StringComparison.OrdinalIgnoreCase))
                {
                    result.Add(word);
                }
            }
